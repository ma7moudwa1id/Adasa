...
export default function BlogsSuggested({post}) {
    const {
    id,
    image,
    title,
  ...
  return (
    <>
      <div
        className={`article relative bg-neutral-900 rounded-xl overflow-hidden group hover:-translate-y-1 transition-transform duration-300`}
      >
        <div className="image relative overflow-hidden">
          ...
          <div className="overlay absolute inset-0 p-3.5 group-hover:bg-linear-to-t group-hover:from-black group-hover:to-70% transition-colors duration-300">
            <div className="label p-1.5 px-3.5 bg-orange-500 rounded-full w-fit text-xs font-semibold">
              {category}
            </div>
          </div>
        </div>
